...
public final class BubbleToastIntegration {
    ...
    private static Optional<ToastData> convertAdvancement(Toast toast) {
        // 26.1 keeps the holder, advancement, and display model public. Use that chain directly
        // so a failed reflective lookup cannot cancel the vanilla toast without a replacement.
        AdvancementHolder holder = readField(toast, AdvancementHolder.class, "advancement", "e");
        Advancement advancement = holder == null ? null : holder.value();
        DisplayInfo display = advancement == null ? null : advancement.display().orElse(null);
        if (holder == null || display == null) {
    ...
    private static Optional<ToastData> convertRecipe(RecipeToast toast) {
        List<?> recipes = readField(toast, List.class, "recipeItems", "recipes", "g");
        ...
        Object entry = recipes.get(0);
        ItemStack icon = readField(entry, ItemStack.class, "result", "item", "stack", "a", "b");
        if (icon == null) {
            icon = invoke(entry, "result", ItemStack.class);
        }
        JsonObject overrides = baseOverrides("bubble_anyway:toast_recipe", "recipe_" + Integer.toHexString(System.identityHashCode(entry)));
        addTextParts(overrides, Component.translatable("recipe.toast"),
                Component.translatable("recipe.toast.description"));
        addItemIcon(overrides, icon);
    ...
    private static void addGenericIcon(JsonObject overrides, Object icon) {
        ...
        if (stack == null) stack = invoke(icon, "getIngredient", ItemStack.class);
        if (stack == null) stack = invoke(icon, "getItemStack", ItemStack.class);
        if (stack != null && !stack.isEmpty()) {
            addItemIcon(overrides, stack);
            return;
        }

        Identifier texture = invoke(icon, "getResourceLocation", Identifier.class);
        if (texture == null) {
            texture = readField(icon, Identifier.class, "texture");
        }
    ...
    private static String textValue(Object value) {
        ...
        Object result = invokeAny(value, "getString", "getText", "asString");
        ...
        while (current != null) {
            for (Field field : current.getDeclaredFields()) {
                ...
                try {
                    ...
                    if (value != null && (value.getClass().getName().contains("Advancement")
                            || hasMethod(value, "value") && hasMethod(value, "id"))) {
                        return value;
                    }
                } catch (ReflectiveOperationException | RuntimeException ignored) {
                }
