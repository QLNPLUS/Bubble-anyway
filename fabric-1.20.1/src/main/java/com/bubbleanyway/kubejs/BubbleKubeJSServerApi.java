...
public final class BubbleKubeJSServerApi {
    ...
    public static void showJson(Object player, String json) {
        ServerPlayerEntity serverPlayer = findServerPlayer(player, 4);
    ...
    private static ServerPlayerEntity findServerPlayer(Object value, int depth) {
        if (value == null || depth <= 0) {
            return null;
        }
        if (value instanceof ServerPlayerEntity serverPlayer) {
            return serverPlayer;
        }
        ...
        for (String methodName : new String[]{"getMinecraftPlayer", "getServerPlayer", "getPlayer", "getEntity"}) {
            ...
            ServerPlayerEntity result = findServerPlayer(nested, depth - 1);
            if (result != null) {
                return result;
            }
        }
