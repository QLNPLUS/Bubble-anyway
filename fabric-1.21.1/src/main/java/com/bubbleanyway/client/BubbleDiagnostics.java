...
public final class BubbleDiagnostics {
    ...
    private static final String LOADER = "Fabric 1.21.1";
    ...
    private static void runReport(MinecraftClient client) {
        try {
    ...
    private static void runToastProbe(MinecraftClient client) {
        if (toastProbeSent) {
            return;
        }
        toastProbeSent = true;
        Identifier id = Identifier.of("bubble_anyway", "diagnostic/fabric_211");
        AdvancementEntry entry = Advancement.Builder.createUntelemetered()
                .display(Items.GRASS_BLOCK, Text.literal("Bubble Anyway diagnostic"),
                        Text.literal("Synthetic Fabric 1.21.1 toast"), null,
                        AdvancementFrame.TASK, true, true, false)
                .build(id);
        client.getToastManager().add(new AdvancementToast(entry));
        LOGGER.info("  toastProbe: queued a synthetic local AdvancementToast");
    }
